...
#else
static int nubia_wakeup_gesture;
#endif
static ssize_t synaptics_rmi4_wake_gesture_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to register input device 11\n",
				__func__);
		...
		bool get)
{
	...
	if (bdata->busregen_gpio >= 0) {
		retval = synaptics_rmi4_gpio_setup(
				bdata->busregen_gpio,
				true, 1, !bdata->busregen_on_state);
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: Failed to configure power GPIO\n",
					__func__);
...
regulator_put:
	...
	if (bdata->busregen_gpio >= 0) {
		retval = synaptics_rmi4_gpio_setup(
				bdata->busregen_gpio,
				false, 1, !bdata->busregen_on_state);
	}
		...
		bool enable)
{
	...
	if (bdata->busregen_gpio >= 0) {
		gpio_set_value(bdata->busregen_gpio, bdata->busregen_on_state);
	}
...
static void resume_work_func(struct work_struct *work)
{
	struct device *dev;
	struct synaptics_rmi4_data *rmi4_data =
		container_of(work, struct synaptics_rmi4_data, resume_work);
